...
// }
// const validateFileType = async(fileUri) => {
...
const handleDocumentSelection = useCallback(async () => {
  try {
    ...
    const filePath = response[0].uri.replace('file://', '');

    // Get the file extension
    const fileExtension = response[0].type
    

    // Define allowed file types
    // const allowedTypes = ['docx', 'pdf', 'jpg', 'jpeg', 'ppt', 'png'];
    const allowedTypes = ['application/pdf', 'image/jpeg', 'image/png', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.ms-powerpoint'];
    // Validate the file type
    const isValidType = allowedTypes.includes(fileExtension.toLowerCase());
      if(isValidType){
       ...
       <ScrollView 
        ...
        className='w-[95%] mx-auto pt-4'>
          ...
          <TouchableOpacity className="items-center rounded-full border border-main" onPress={handleDocumentSelection}>
           <Text className="text-main  text-[18px] font-ageomedium py-3  px-12  ">Select File</Text>
          </TouchableOpacity>
            
          <Text className="text-main text-center  text-[18px] font-ageomedium py-4 px-12  ">{filename} </Text>
          {
            filename && 
            <TouchableOpacity
...

        {
          ...
          {/* name */}
          {/* <View className='py-2'>
            <Text className='text-[18px] font-ageomedium text-grey-800'>Name</Text>
          ...
          {/* type */}
          <View className='py-2'>
            <Text className='text-[18px] font-ageomedium text-grey-800'>Type</Text>
            <Dropdown  
          ...
          <View className='py-2'>
             <Text className='text-[18px] font-ageomedium text-grey-800'>Department</Text>
             <Dropdown  
          ...
          {/* level */}
          <View className='py-2'>
            <Text className='text-[18px] font-ageomedium text-grey-800'>Level</Text>
            <Dropdown  
        ...
        <TouchableOpacity className="items-center mt-4 rounded-lg bg-main justify-end " onPress={handleUpload}>
            <Text className="text-lightmain text-[18px] font-ageomedium py-4 px-12  " >Upload</Text>
          </TouchableOpacity>
